...
function loadVerificationCodes() {
    try {
        ...
        console.error('⚠️ Error cargando códigos:', error);
        ownerCodes = [];
        adminCodes = [];
    }
}
...
function markCodeAsUsed(code, type) {
    ...
    console.log(`⚠️ No se pudo marcar código ${code} como usado`);
    return false;
}
    ...
    return new Promise((resolve, reject) => {
        ...
        script.onload = () => {
            try {
                ...
                console.error('⚠️ Error inicializando EmailJS:', error);
                reject(error);
            }
        };
        script.onerror = (error) => {
            console.error('⚠️ Error cargando EmailJS desde CDN:', error);
...
async function sendEmailViaEmailJS(formData) {
    try {
        ...
        console.error('⚠️ Error detallado enviando email:', error);
...
async function sendRequestToDevs(formData) {
    ...
    try {
        ...
        console.error('⚠️ EmailJS falló, usando fallback:', error);
...
function setupEventListeners() {
    ...
    document.getElementById('propietarioVerificationCode').addEventListener('input', validatePropietarioCodeInput);
    document.getElementById('adminVerificationCode').addEventListener('input', validateAdminCodeInput);
}

// CORREGIDO: Validación de código de propietario en tiempo real
function validatePropietarioCodeInput() {
...
// CORREGIDO: Validación de código de administrador en tiempo real
